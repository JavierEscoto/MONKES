module DKE_BTD_Solution_Legendre_Orthonormal 
   ...
   public :: Solve_BTD_DKE_Legendre_Orthonormal  
 ...
 contains  
   ...
   subroutine Solve_BTD_DKE_Legendre_Orthonormal( N_theta, N_zeta, N_xi, nu, E_r, &
      ...
      Sigma_1(:,:,0) = S1(:,:,0) 
      Sigma_1(:,:,2) = S1(:,:,2) 
      S3 = 0 ; S3(:,:,1) = B(0:N_theta-1,0:N_zeta-1) *sqrt(2d0/3) ; Sigma_3 = S3(:,:,0:2)
    ...
    contains
      ...
      subroutine Compute_monoenergetic_coefficients( Gamma, Gamma_33_Spitzer )
        ...
        real :: vm_Leg(0:N_theta,0:N_zeta,0:2)         
        ...
        ! in periodic grid with repeated point.  They have \norm{P_k} in
        ! the denominator to translate the orthonormal Legendre modes to Legendre modes
        do j = 0, N_zeta
           do i = 0, N_theta
        ...
        Gamma_11 = Integral( ["theta","zeta"], g * ( -vm(:,:,0) * U1(:,:,0) - vm(:,:,2) * U1(:,:,2))  ) / V_prime
        Gamma_31 = Integral( ["theta","zeta"], g * sqrt(2d0/3) *U1(:,:,1) * B ) / V_prime 
        Gamma_13 = Integral( ["theta","zeta"], g * ( vm(:,:,0) * U3(:,:,0) + vm(:,:,2) * U3(:,:,2) )  ) / V_prime
        Gamma_33 = Integral( ["theta","zeta"], g * sqrt(2d0/3) * B * U3(:,:,1)) / V_prime  
  ...
  ! of monoenergetic coefficients. 
   subroutine Set_DKE_Grids_and_coefficients( nu, E_psi, &
      ...
      subroutine Set_DKE_Coefficients
        ...
        real :: C(0:N_xi,-1:1) ! normalization factor for coefficients in orthonormal representation
        ...
        do k = 0, N_xi
           ...
           C(k,-1) = sqrt( (2d0*k-1) / (2d0*k+1)  ) ; if(k==0) C(k,-1) = 0
           C(k,0)  = 1
           C(k,+1) = sqrt( (2d0*k+3) / (2d0*k+1)  )
            
           a_zeta(:,:,k,-1) = C(k,-1)* k * 1d0 / ( g*B * (2*k-1)) 
           a_zeta(:,:,k,0)  = E_psi * B_theta / ( g * B2_mean )
           a_zeta(:,:,k,+1) = C(k,+1)*(k+1) * 1d0 / ( g * B * (2*k+3)) 
           
           a_theta(:,:,k,-1) = C(k,-1)* k * iota / ( g*B * (2*k-1)) 
           a_theta(:,:,k,0)  = -E_psi * B_zeta / ( g * B2_mean )  
           a_theta(:,:,k,1)  = C(k,+1)*(k+1) * iota / ( g * B * (2*k+3))           
		   
           a(:,:,k,-1) = C(k,-1)* (k*(k-1)) * ( dBdzeta + iota * dBdtheta )      &
                       / ( 2*(2*k-1) * g * B**2 ) 
           a(:,:,k,+1) = -C(k,+1)*((k+1)*(k+2)) * ( dBdzeta + iota * dBdtheta ) &
                       / ( 2*(2*k+3) * g * B**2 )   
           a(:,:,k,0)  =  0.5*nu * k * (k+1)              
        end do            
        
        ! *** Legendre modes (non-orthonormal) of the magnetic radial drift 
        !  vm(:,:,k) = vds_l * ( 2 + P_2(xi) ) / 3
        allocate( vm(0:N_theta,0:N_zeta,0:N_xi) )
        vm = 0
        vm(:,:,0) = ( vds_l * 2 / 3 )* sqrt(2d0) 
        vm(:,:,2) = ( vds_l / 3    ) * sqrt(2d0/(2d0*2+1))
   ...
   ! Solves the monoenergetic DKE using an algorithm for inverting
   ! block tridiagonal matrices. N_xi must be greater or equal to 2
   subroutine Solve_BTD_DKE_Legendre_Orthonormal_OLD( N_theta, N_zeta, N_xi, nu, E_rho, &
                                       Gamma, Gamma_33_Spitzer )
                                       !Gamma_11, Gamma_31, Gamma_13, Gamma_33 )
      integer, intent(in) :: N_theta, N_zeta, N_xi
    ...
    contains
      ...
      subroutine Forward_elimination
        ...
        ! Computation of Delta_2 and L_2
        do k = N_xi-1, 2, -1
           ...
           ! Extract Delta_2^{-1} for Backward substitution and U_2
